  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-nykaa-text tracking-tight">Order <span className="text-pink-500">Center</span></h1>
          <p className="text-nykaa-text-muted font-medium mt-1">Processing {orders.length} orders from your customers.</p>
        </div>
      ...
      >
        {filteredOrders.map((order) => (
          <motion.div 
            ...
            className="glass-card group hover:bg-nykaa-surface/5 transition-all p-6"
          >
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div className="flex items-start gap-6">
                 ...
                 <div className="space-y-1">
                    <div className="flex items-center gap-3">
                       <h3 className="text-lg font-black text-nykaa-text tracking-tight">#{order._id.slice(-8).toUpperCase()}</h3>
              ...
              <div className="flex items-center gap-8">
                 ...
                 <div className="h-12 w-px bg-nykaa-border hidden lg:block"></div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button className="glass size-12 flex items-center justify-center rounded-none text-nykaa-text-muted hover:text-nykaa-text transition-all active:scale-90">
            ...
            <div className="mt-6 pt-6 border-t border-nykaa-border grid grid-cols-2 md:grid-cols-4 gap-4">
               {order.orderItems?.slice(0, 4).map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 bg-nykaa-surface/5 p-3 rounded-none">
                     <img src={item.image} alt="" className="size-10 rounded-none object-cover" />
                     <div className="min-w-0">
                        <p className="text-[10px] font-black text-nykaa-text truncate">{item.name}</p>
                        <p className="text-[10px] font-bold text-nykaa-text-muted">x{item.qty}</p>
                     </div>
                  </div>
               ))}
               {order.orderItems?.length > 4 && (
                  <div className="flex items-center justify-center bg-nykaa-surface/5 p-3 rounded-none text-[10px] font-black text-nykaa-text-muted hover:text-nykaa-text cursor-pointer transition-colors">
        ...
        {filteredOrders.length === 0 && (
          ...
          >
            <ShoppingCart size={48} className="text-nykaa-text-muted mx-auto mb-4" />
            <h3 className="text-xl font-black text-nykaa-text">No orders yet</h3>
            <p className="text-nykaa-text-muted mt-2">When customers start ordering, they'll appear here.</p>
          </motion.div>
        )}
      </motion.div>
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-3xl font-black text-nykaa-text mb-3 tracking-tighter">Delete Order?</h3>
              <p className="text-nykaa-text-muted font-medium mb-10 leading-relaxed italic">
                 Are you sure you want to delete order <span className="text-nykaa-text font-black not-italic">"#{selectedOrder?._id.slice(-8).toUpperCase()}"</span>? This will permanently remove it from your history.
              </p>
